...
class WeekEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		missingFileText.setFormat(Paths.font("font.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		txtTracklist.font = Paths.font("font.ttf");
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	override function update(elapsed:Float) {
		...
		if(iconInputText.hasFocus) {
			...
			if(controls.UI_DOWN_P || controls.UI_UP_P) changeSelection(controls.UI_UP_P ? -1 : 1);
